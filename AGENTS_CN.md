...
| 进程间控制 | `logs/control_command.json` 文件命令（开始/停止录音、暂停/恢复监听），由托盘守护进程消费 |
...
设置界面 (PySide6)  python -m voicecontrol.ui.settings_app（托盘菜单也可打开）
开机自启            托盘菜单开关（HKCU Run）
TTS 状态播报        「我在」「请说」「正在识别」… 随流水线状态触发
命令历史            logs/command_history.jsonl 追加写入
诊断工具            设置页：麦克风 / VAD / 唤醒词测试、日志查看
...
.venv\Scripts\python.exe -m voicecontrol.ui.settings_app      # 设置 / 诊断界面
```

日志（托盘模式）：`logs/YYYYMMDD_voicecontrol.log`（按自然日一个文件）。
...
├── logs/                       按日托盘日志、command_history.jsonl、diagnostics.jsonl、
│                               control_command.json（git 忽略）
...
- `events/` — 进程内状态 pub/sub，连接 pipeline、托盘、TTS、设置 UI。
- `history/` — 追加式命令历史；设置页可重发上一条。
- `diagnostics/` — 自测工具，在设置 UI 中暴露。
- `tts/` — Windows SAPI 封装与状态短句订阅。
- `ui/` — PySide6 设置/诊断窗口；不含 pipeline 逻辑。
- `utils/` — 仅放无处归属的代码。
